...
**`TZ=Asia/Seoul` 필요**: 화면이 서버에서 그려지므로 '오늘의 소비'·캘린더가 서버의 '오늘'을 쓴다.
Vercel 기본은 UTC라 이 값이 없으면 한국 시간 오전 9시 이전에 하루 어긋난 화면이 나온다.
**로컬 PC는 이미 KST라 재현되지 않는다** — 배포 환경 설정에서만 드러나는 문제다.
