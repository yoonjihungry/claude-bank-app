...
단색 톤을 흰 배경 + 얕은 회색 표면 + 토스 블루 `#3182f6` 포인트 하나의 미니멀 톤으로 다시 잡음.

- `styles/tokens.css`: `:root` 색 토큰 교체. 배경/카드 순백(`0 0% 100%`), 카드 안 구획은
  `--muted`(`210 20% 98%`) 회색 표면으로, 잉크 `#191f28`·보조 `#8b95a1`, 포인트
  `--primary`/`--accent`/`--ring` = `#3182f6`, 경고·삭제 `#f04452`, 주말 일=레드·토=블루.
  수입/지출/신용은 색 없이 잉크 무채색으로 통일(방향은 +/− 부호·굵기로만). `--kakao`/`--naver`/`--desk-*`는 보존.
- `screens/MyPage.tsx`: 수입/지출/잔액 요약을 3열 그리드(`grid-cols-3`)에서 세로 리스트로 교체.
  좁은 칸에서 큰 금액이 줄바꿈으로 깨지던 문제 해결. `SummaryCell`(가운데 3단) → `SummaryRow`
  (왼쪽 라벨+전월 대비 / 오른쪽 금액, 항목 사이 `divide-y`)로 재작성 — 시안 그대로.
...
**이번 달 회차분만**(청구 기준)이라 두 숫자의 기준이 어긋났다. 예) 60만원 6개월 할부 → 소비 60만원 /
청구 10만원. 소비를 청구 기준으로 통일해, 실제 이번 달 빠져나가는 돈과 일치시켰다.

- `useStatistics.ts`: 청구액 계산을 `billForMonth(tx, targetIdx)` 헬퍼로 뽑아, 지출 집계 전체
  (`totalExpense`·`expenseByCategory`·`expenseByMethod`·`budgetUsage`·`avgDailyExpense`·`savingsRate`)를
  청구 기준으로 계산한다. 현금·체크·일시불은 거래한 달에 전액, 할부는 이번 달 회차분만(다른 달에 산
  할부의 이번 달 회차도 포함). `creditBillingTotal`은 이제 `totalExpense`의 부분집합.
- **일별 순액·추이·캘린더(`dailyTrend`/`dailyNet`)는 거래일 기준 유지** — 할부를 미래 날짜로 흩을 수
  없어 실제 거래일을 반영한다. `creditCardTotal`(구매 기준 신용 전액)은 참고용으로 남겨둠.
- 대시보드·예산·통계 화면은 모두 `useStatistics`를 통해 값을 받으므로 컴포넌트 수정 없이 반영된다.
...
- `HeaderAuth`·`ProfileCard`: 프로필 `<img>`에 `referrerPolicy="no-referrer"` 추가 —
  Google 이미지 서버(`lh3.googleusercontent.com`)가 referrer 붙은 요청에 429를 돌려줘 로드가
  실패하던 것을 회피. 그래도 실패하면 `onError`로 이니셜 아바타 폴백으로 전환(깨진 아이콘 방지).

## 2026-07-29 — 마이·차트 단색화 + 고정거래/디테일 정리 (리디자인 4화면 완료)

무채색+파랑 단색 시스템을 마지막 화면(마이)과 차트까지 적용해, 홈·거래·카테고리·마이 4개 화면 통일 완료.

- `utils/chartColor.ts` 신설(`blueRamp`/`rankedBlueByKey`/`SERIES_EXPENSE·INCOME`): 차트의 여러 색을
  **파랑 한 색의 농담**으로. `CategoryChart` 도넛·`PaymentMethodBar` 스택 = **큰 값일수록 진한 파랑**,
  `MonthlyTrendChart` = 지출 진파랑/수입 연파랑, `DailyTrendChart`는 이미 파랑(카드 테두리만 정리).
- `MyPage`: 프로필·요약·지표 카드 테두리 제거(흰 카드+그림자), 저축률=파랑 강조, 섹션 제목 톤 통일.
  `ProfileCard` 로그인 유도 이모지(👤) → 인라인 SVG.
- 홈 '오늘 소비' 행에 남아 있던 회색 카테고리 아이콘 제거.
- 고정거래(`RecurringPanel`): 무지개 색점 제거 + 개별 테두리 카드 → **통합 카드(구분선)**,
  '고정거래 직접 추가' 버튼 점선 스타일로 정리.
- 예산 미설정 안내 문구 축약("예산 미설정 — 톱니바퀴로…" → "예산 미설정").

## 2026-07-29 — 단색 전환: 앱 전체를 '무채색 + 파랑 하나'로 통일 (+ 카테고리 재구성)

카카오페이 옐로 톤 직후 "무지개색·상태색 때문에 촌스럽다"는 피드백 → 색을 다 빼고 포인트 색
하나로 통일하기로. 후보(블루/청록/인디고) 비교 후 **파랑 #2f6bff** 확정. 토큰 한 곳을 바꿔
앞서 적용한 옐로/청록/초록/빨강을 한 번에 정리했다.

- `tokens.css`: `--primary`·`--accent`·`--ring`·`--warning`·`--hero-*` = `222 100% 59%`(파랑),
  `--accent-foreground` = 흰색. 옐로 accent가 파랑으로 바뀌며 홈·거래의 CTA/FAB/뱃지/진행바가 일괄 전환.
  **의미색 제거**: `--income`·`--expense`·`--credit`를 잉크(`60 20% 9%`)로 통일 → 수입/지출은 색이 아니라
  **+/− 부호로만** 구분. `--destructive`(빨강)만 삭제 등 위험 동작용으로 남김.
- **카테고리 재구성**(`BudgetPage`/`BudgetPanel`): 상단 **'이번 달 예산 사용' 요약 카드** 추가,
  카테고리들을 **한 흰 카드에 줄 목록**으로 통합 — **색점 제거·파랑 단색 진행바·초과는 잉크 태그**,
  상태색(초록/주황/빨강) 미사용. 추가 버튼 파랑. 긴 설명 문단 제거.
- 홈 '오늘 소비' 행의 카테고리 색점도 무채색으로.
- **남은 것**: 마이 페이지 차트(`CategoryChart` 파이·`DailyTrendChart` 등)는 아직 데이터/토큰 색을
  써서, 파랑 한 색의 농담(진하기)으로 정리 예정.

## 2026-07-29 — 거래·홈을 카카오페이 톤으로 리디자인 (옐로 포인트·흰 카드·검정 볼드 숫자)

기존 화면이 "밋밋하다/톤이 안 맞는다"는 피드백으로 카카오페이(kakaopay.com) 톤에 맞춰
거래·홈을 다시 그렸다. 데이터·집계·기능은 그대로 두고 배치·표현만 바꿨다.

- **공통 톤**: 배경 회색(`--background` 220 13% 96%) 위에 **흰 카드 + 그림자**(테두리 지양),
  포인트 = 옐로 `--accent`(#fee500, index.css `@theme`에 `--color-accent(-foreground)` 매핑).
  **옐로는 CTA·FAB·작은 뱃지·진행바 같은 '배경'으로만, 그 위 글자는 근검정(`--accent-foreground`).**
  큰 면을 통째로 노랑으로 칠하지 않는다(홈 히어로 전체 노랑은 반려됨). 금액 = 숫자 크게 + 뒤에 작은 "원",
  지출은 잉크(검정)·수입만 초록. 아이콘은 인라인 SVG. 하단 nav에 위쪽 그림자.
- **거래**: 입력 폼을 상단에서 빼 우하단 **옐로 FAB → 바텀시트**(`TransactionSheet`). 목록은
  **하루=흰 카드 하나**로 묶고(`TransactionGroupList`) 행 탭→수정 시트(삭제 포함). 상단은
  **월 지출 요약 카드 + pill 필터(전체/지출/수입) + '필터'→`FilterSheet`**(기간·금액·검색·고정숨기기
  보존, `FilterBar`에 `bare` prop 추가).
- **홈**: 히어로를 **흰 카드 + 검정 숫자**로(옐로는 뱃지·진행바 작은 포인트만). 히어로 아래
  **빠른 메뉴 4개**(`QuickMenu`: 기록·통계·예산·고정), **'오늘 소비'를 위로**, **'이번 달 한눈에'**
  (`MonthGlanceCard`)로 소비·수입·카드청구를 **한 카드 3줄 요약**(각 줄 탭→상세). 이에 따라
  `IncomeSummaryLine`·`MonthlySpendingCard`·`CreditBillingCard`는 **홈에서 미사용**(파일은 보존 —
  Top3 막대·현금/신용 분해는 추후 '소비' 상세로 이설 여지).
...
배치와 표현만 바꿨다(위=매번 보는 상태 → 아래=궁금하면 보는 상세).

- 구성: **예산 잔액 히어로 → 이번 달 수입(슬림) → 이번 달 소비 → 카드 청구 예정 → 오늘 내역**.
- 신규 `BudgetHeroCard`(쓸 수 있는 돈 = 예산 합계 − 지출, 진행바·격려 뱃지. 예산이 없으면
  '이번 달 소비'로 폴백), `IncomeSummaryLine`, `TodayTransactions`, `HighlightTitle`(형광펜 밑줄 제목).
- `MonthlySpendingCard`: 총액 반복을 빼고 **카테고리 Top3 막대 + 현금·체크/신용(이달 구매) 분해**.
  `CreditBillingCard`: 파스텔 카드·아이콘 칩·"이달 청구" 라벨로 정리.
- 예산 합계는 `useStatistics.budgetUsage`의 limit 합으로 계산(새 집계 훅 없음). 파스텔 톤은 새 색 없이
  기존 시맨틱 토큰의 투명도 유틸(`bg-expense/10` 등)로 내고, 카테고리 색은 데이터(hex)를 그대로 쓴다.
- 홈에서 캘린더(`TransactionCalendar`·`SelectedDayPanel`)와 월 네비게이터를 내렸다(파일은 보존 —
  [decisions.md](decisions.md) 참조). `TodaySpendingCard`는 히어로로 대체돼 미사용.
- nav(하단 탭바)는 `AppShell` 소관이라 이번 변경에서 제외(홈 본문만 교체).

## 2026-07-27 — 마이페이지 보강(계정 카드·핵심 지표·수입 시각화·결제수단)

마이페이지가 지출 위주에다 거래 페이지(카테고리 파이·일자별)와 겹쳐 얇았다. "종합" 성격을 더했다.

- **계정 카드**(`ProfileCard`, 최상단): 로그인 시 아바타·이름·이메일·로그아웃, 비로그인 시
  로그인 유도(눌러 `LoginSheet`). '마이'의 정체성을 채운다.
- **핵심 지표**: 저축률(잔액÷수입)·하루 평균 지출을 요약 아래 2칸으로. `useStatistics`에
  `savingsRate`·`avgDailyExpense`(당월은 오늘까지 경과일, 지난달은 그 달 전체 일수로 나눔) 추가.
- **결제수단별 지출**(`PaymentMethodBar`, 신규): 현금/체크/신용/미지정을 가로 스택 바 + 범례로.
  `useStatistics.expenseByMethod` 집계 추가. 색은 토큰 배정(현금=warning·체크=primary·신용=credit·미지정=회색).
- **수입 시각화**: 최근 6개월 차트(`MonthlyTrendChart`)를 지출만 → **수입·지출 그룹 막대**로 바꾸고
  월 평균 수입/지출을 함께 표시. 카테고리 차트에 **지출/수입 토글**을 달아(`CategoryChart`에 `kind`
  prop, `useStatistics.incomeByCategory` 추가) 수입 카테고리도 본다. `CategoryChart`는 거래 페이지도
  쓰지만 `kind` 기본값이 'expense'라 그쪽은 종전과 같다.

## 2026-07-27 — 마이페이지 일자별 추이에 주차(週) 구간 선택 추가

마이페이지의 "일자별 수입·지출 추이"(막대+누적선 `DailyTrendChart`)는 한 달치를 좁은 폭에
전부 그려 막대·X축 라벨이 겹쳤다. 차트는 그대로 두고 **한 번에 보는 구간을 주 단위로 좁혔다**.

- 차트 위에 `[전체][1주][2주]…` **칩**을 두고, 선택한 주의 날짜만 그래프에 넘긴다. 주는
  7일 블록(1~7 / 8~14 / …)이라 달의 일수에 따라 칩이 4~5개 생긴다. 선택 주가 있으면 제목 옆에
  날짜 범위(예 `7/8 ~ 7/14`)를 캡션으로 보여주고, 그 주에 거래가 없으면 "이 주는 거래가 없어요".
- 기본 선택은 **현재 월이면 오늘이 속한 주**, 과거/다른 달이면 "전체". 월을 넘기면 그 달 기준으로
  다시 잡는다.
- 필터는 마이페이지에서만 하고 `useStatistics`는 손대지 않았다 — 거래 페이지의 같은 차트는
  종전대로 한 달 전체를 그린다.
- 좁은 구간에서 순액 막대와 누적선을 함께 그리니 둘의 스케일이 안 맞아(막대는 하루치, 누적선은
  월 1일부터의 합) 막대가 희미하게 눌려붙고 서로 따로 놀았다. `DailyTrendChart`에 옵션 `lineOnly`를
  더해, 켜면 **막대를 빼고 누적 순액 꺾은선 하나만** 그린다. 마이페이지는 이 옵션을 써 선만 그리고
  (점이 8개 이하일 때만 점별 금액 라벨 표시), 거래 페이지는 기본값(false)이라 막대+선을 그대로 유지한다.

  (앞서 같은 문제를 지출 히트맵 달력으로 풀어봤으나 "원래 그래프가 낫다"는 피드백으로 되돌리고
  구간 선택 방식으로 바꿨다.)

## 2026-07-24 — 도넛 중앙 라벨 위치 · 바텀시트 그림자 누출 수정

배포된 화면에서 눈에 띈 렌더링 버그 두 건을 고쳤다.

- **도넛 중앙 라벨**(`CategoryChart`): 총 지출 텍스트가 도넛 한가운데가 아니라 왼쪽 위로 밀려
  잘려 나왔다. Recharts `<Label>`이 넘겨주는 `viewBox`에서 `cx/cy`를 못 받아 `0`으로 떨어진 탓.
  **SVG `<Label>`을 걷어내고 차트 컨테이너(`relative`) 위 HTML 오버레이**(`absolute inset-0`
  + flex 중앙 정렬, `pointer-events-none`)로 바꿔 좌표 의존을 없앴다. 색도 런타임 `tokenColor`
  대신 토큰 유틸리티 클래스(`text-muted-foreground`/`text-foreground`)를 쓴다.
- **바텀시트 그림자 누출**(`DatePickerField` / `PeriodPickerField` / `LoginSheet`): 닫힌 시트의
  그림자가 하단 탭바 위에 회색 띠로 남았다. 시트는 애니메이션 때문에 닫혀도 마운트된 채
  `translate-y-full`로 밀려날 뿐인데, 그림자가 `0 -10px 40px -12px`로 **위로 퍼지는** 값이라
  몸통만 화면 밖으로 나가고 그림자는 시트 윗변에서 계속 번졌다. 시트 컨테이너가 `z-50`이라
  `z-30`인 탭바를 덮어 더 두드러졌다. **`open`일 때만 그림자를 칠하도록** 고쳤다 — 스크림이 없는
  닫힌 상태에선 그릴 이유가 없다. (`MigrationSheet`는 필요할 때만 마운트돼 해당 없음.)
- 교훈: 상시 마운트되는 오버레이는 **닫힌 상태에서 박스 경계를 넘는 속성**(그림자·아웃라인·필터)이
  새지 않는지 함께 확인한다. transform은 몸통만 옮기고 그림자는 따라 나가지 않는다.

## 2026-07-24 — 마이페이지 그래프 리디자인(둥근 막대·도넛)

학습앱 시안 스타일을 참고해 마이페이지 두 차트의 시각 디자인을 바꿨다. 데이터·집계는 그대로고
표현만 교체했다. (사전 목업으로 방향 확인 후 반영.)

- **최근 6개월 지출**(`MonthlyTrendChart`): 수입·지출 2계열 막대 → **지출 1계열**로 좁히고,
  둥근 pill 막대(`radius={8}`)로 바꿨다. **기준(마지막) 달만 `--primary`로 강조 + 값 말풍선**을
  띄우고 나머지 달은 `--muted` 회색(`<Cell>` per-bar). 차트 아래 **월 평균 지출** 요약 행 추가.
  (섹션 제목도 "최근 6개월 추세" → "최근 6개월 지출".)
- **카테고리별 지출**(`CategoryChart`): 파이 → **도넛**(가운데 뚫고 `<Label>`로 총 지출액 표시),
  하단 기본 범례 대신 **오른쪽 커스텀 범례**(색·이름·비율%).
- **포맷터**(`utils/format`): 좁은 자리용 `formatWonCompact` 추가(만/억 축약, 예 `185만원`).
- 색은 전부 tokens.css 토큰(`tokenColor`)만 사용, 하드코딩 없음.

## 2026-07-24 — 마이페이지(통계) 추가

하단 탭바에 네 번째 탭 **마이**(`/mypage`)를 카테고리 오른쪽에 추가하고, 계정 정보와
통계를 한 화면에 모았다.

- **탭바**(`AppShell`): `UserIcon` 추가, `TABS`에 `/mypage` 등록, 하단 탭 그리드
  `grid-cols-3` → `grid-cols-4`.
- **라우트/화면**(`app/(shell)/mypage/page.tsx` 래퍼 + `screens/MyPage` 신규):
  ① `MonthNavigator`로 기준 월 이동
  ② 이번 달 수입/지출/잔액 요약 + **전월 대비 증감**
  ③ 최근 6개월 추세 · ④ 카테고리별 지출 · ⑤ 일자별 추이.
  (계정 정보/로그아웃은 헤더 아바타 메뉴에 그대로 있어 마이페이지에는 두지 않는다.)
- **집계**(`hooks/useMonthlyTrend` 신규): 기준 월 기준 최근 N개월 월별 수입/지출을 `useMemo`로
  집계해 전월 대비와 추세 차트 데이터를 함께 파생한다(집계는 훅에 둔다는 규칙 준수).
- **추세 차트**(`components/MonthlyTrendChart` 신규): 월별 수입·지출 막대(Recharts,
  `--income`/`--expense` 토큰 색). 카테고리 파이·일자별 추이 차트는 기존 컴포넌트를 재사용한다.
...
- **선택일 패널 시안 정합**(`SelectedDayPanel`·`TransactionList`): 뉴스캐시 시안대로 행을
  다듬었다. 제목은 `메모 · 카테고리` 한 줄, 결제수단 뱃지는 이름 **아래**로(같은 날 앞선 항목에서
  이름 오른쪽으로 옮겼던 걸 시안 기준으로 되돌림), 금액은 `formatSignedWon`으로 `₩` 대신 `원`
  표기 통일(거래 페이지 목록도 함께). `TransactionList`의 `hideColorDot`/`hideDate`는 `flush`
  하나로 합쳤다.
- **선택일 패널 아코디언화**(`SelectedDayPanel`): 헤더의 '닫기'가 섹션을 통째로 없애던 걸,
  헤더를 눌러 본문(요약+내역)을 접었다 폈다 하는 아코디언으로 바꿨다. 캘린더 선택은 유지되고,
  다른 날짜를 고르면 다시 펼친다. 높이는 CSS `grid-template-rows: 0fr↔1fr` 트랜지션으로만 처리.
- **거래 목록 기본 '이번 달'**(`TransactionsPage`): 필터가 비어 전체 기간을 쏟아내 매달 쌓이는
  고정거래로 목록이 지저분하던 걸, 진입 시 이번 달만 보도록 기본값을 `{ month: currentMonth() }`로
  뒀다. 다른 달은 기존 월 선택기로 이동.
- **고정거래 숨기기 토글**(`FilterBar`·`useTransactions`): 보조 수단으로 필터에 `hideRecurring`을
  추가. 체크하면 반복 규칙이 자동 생성한(`recurringId`) 거래를 목록에서 숨긴다(기본 꺼짐).

## 2026-07-24 — 모달·목록 UX 다듬기 + 고정거래 소급 방지

대시보드 리뉴얼 후속으로 손에 걸리던 잔가지를 정리했다.

- **모달 가운데 정렬 + 배경 스크롤 잠금**(`CategoryModal`·`RecurringModal`): 바텀시트가
  화면 아래에 붙어 열릴 때 입력칸 포커스마다 페이지가 밀려 화면이 튀었다. 항상 화면 중앙에
  띄우고, 열려 있는 동안 `body` 스크롤을 잠근다. 잠금 로직은 재사용 훅 `useBodyScrollLock`
  (`overflow:hidden` + 스크롤바 폭만큼 패딩 보정)으로 분리.
- **헤더 로고 → 홈 링크**(`AppShell`): `₩ 가계부`를 누르면 `/`로 이동한다.
- **고정거래 소급 생성 방지**(`RecurringModal`·`utils/recurring.ts`): 새 규칙을 넣으면
  시작월부터 이번 달까지 지난 달치가 한꺼번에 내역에 쏟아졌다. 새 규칙의 `generatedMonths`를
  `monthsBeforeCurrent()`로 이번 달 직전까지 미리 채워, 이번 달부터만 생성한다(앞으로만 쌓임).
  수정·서버(`/api/recurring/run`) 동작은 그대로.
- **선택일 패널 목록 정리**(`SelectedDayPanel`·`TransactionList`): 카드(패널) 안에 행마다
  또 카드가 겹쳐 '박스 속 박스'로 보이던 걸 `flush` 옵션(구분선만 있는 납작한 목록)으로 없앴다.
  결제수단 뱃지(체크·현금·카드)는 카테고리 이름 아래줄 → 이름 오른쪽으로 옮기고, 날짜·할부·고정
  아래줄은 내용이 있을 때만 그린다(거래 페이지 목록은 카드·날짜 그대로).

## 2026-07-23 — 대시보드 디자인 리뉴얼 (뉴스캐시 시안 반영)

gdweb 뉴스캐시(SBS) 스타일을 참고해 대시보드 스킨을 바꿨다. 기존 항목·데이터는 그대로 두고
표현만 바꿨다(기능 변경 없음).

- **오늘의 소비**(`TodaySpendingCard`): 흰 카드 → 파란 그라데이션 히어로. 색은 하드코딩 대신
  `tokens.css`에 `--hero-from/mid/to` 3-스톱 트리플릿을 추가하고 컴포넌트에서 `hsl()`로 합성.
  히어로 위 텍스트는 `text-primary-foreground`, 비교 배지는 `bg-primary-foreground/20`.
  돈주머니 아이콘 대신 증감 방향 화살표(↑/↓)만 표시.
- **하단 탭바**(`AppShell`): 텍스트만 있던 탭에 라인 아이콘 추가(홈=집, 거래=교환 화살표,
  카테고리=2×2 격자). NH앱 nav의 얇은 아웃라인 스타일 참고. 활성 탭 상단에 인디케이터 바.
- **헤더**(`AppShell`): 💰 이모지 → ₩ 마크(둥근 사각형)+"가계부". 로그인 버튼(`HeaderAuth`)은
  파란 알약(`bg-primary/10 text-primary`)으로.
- **이번달 소비금액**(`MonthlySpendingCard`): 현금·체크/신용카드 분해를 인라인 텍스트 →
  두 개의 타일(`bg-muted/50`)로.
- **선택일 패널**(`SelectedDayPanel`): 각 거래 행의 좌측 색 점·날짜를 숨겼다. 패널 헤더에 이미
  날짜가 있어 중복이라 시안처럼 이름·결제수단·금액만 남긴다. 공유 컴포넌트 `TransactionList`에
  `hideColorDot`/`hideDate` 옵션을 추가해 이 패널에서만 적용(거래 페이지 목록은 그대로).
- 신용카드 청구 카드·캘린더는 이미 시안과 같은 표현이라 손대지 않았다.

## 2026-07-21 — 로그인 사용자의 첫 화면을 서버에서 그린다 (새로고침 체감 개선)

8-3/8-4 이후 새로고침하면 화면이 한참 비어 있다가 채워졌다. 원인은 대기가 순서대로 쌓인 것.

```
1. 브라우저가 JS 받고 실행          ← 이 동안 화면 비어 있음
2. GET /api/auth/session   ~750ms  ← 로그인 여부를 알아야
3. 데이터 4종 병렬 조회     ~2.8s   ← 어디서 읽을지 정해진다
4. POST /api/recurring/run ~1.3s
```

2번이 끝나야 3번을 시작할 수 있는 구조였다. 서버는 1번 시점에 이미 답을 알고 있으므로 실어 보냈다.

- `layout.tsx`가 `auth()`로 세션을, `lib/ledgerData.ts`로 가계부 데이터를 읽어
  `AuthProvider`/`AppShell`에 넘긴다. → 위 2·3번이 사라진다.
- `LedgerProvider`가 `initialData`로 시작한다. 첫 렌더부터 화면이 채워져 로딩 문구를 볼 일이 없다.
- `AppShell`의 마운트 대기는 **비로그인일 때만** 적용한다. 로그인 사용자는 서버 HTML과
  클라이언트 첫 렌더가 같은 값을 쓰므로 기다릴 이유가 없다.
- 고정거래 규칙이 하나도 없으면 `/api/recurring/run`을 호출하지 않는다(위 4번 절약).

**라우트 그룹 도입**: 처음엔 루트 레이아웃에서 세션·데이터를 읽었는데, 그러면 가계부와 무관한
`/desk`까지 매 요청 DB를 읽고 정적 렌더를 잃는다. `(shell)` 그룹을 만들어 가계부 화면
(`/`·`/transactions`·`/budget`)만 그 안에 넣었다. 주소는 그대로다.
`AppShell`의 `BARE_ROUTES` 배열도 함께 제거했다 — 런타임 분기로는 레이아웃이 이미 한
서버 작업을 되돌릴 수 없어서, 애초에 거치지 않게 하는 편이 맞다.

**대가**: 가계부 화면이 요청 시점 렌더(`ƒ`)가 됐다. 사람마다 내용이 다르니 당연한 귀결이다.
`/desk`는 정적(`○`)으로 남는다.

**하이드레이션 불일치 수정**: `TodaySpendingCard`가 '기준 시각'을 초 단위로 찍는데, 서버가 그린
시각과 브라우저가 붙는 시각은 반드시 어긋난다(`14:23:41` vs `14:23:42`). 시간대 설정으로는
해결되지 않는 종류라, 초기값을 `null`로 두고 마운트 후에 채우도록 바꿨다 —
'마지막으로 확인한 시각'은 브라우저에서만 의미가 있으므로 서버에서는 아예 그리지 않는다.

**'오늘'의 기준을 한국 시간으로 고정**: 화면이 서버에서 그려지므로 '오늘의 소비'·캘린더가
서버가 아는 '오늘'을 쓴다. 배포 서버는 UTC라 그대로 두면 한국 시간 오전 9시 이전에
하루 어긋난 화면이 나온다(로컬 PC는 KST라 재현되지 않는다).

처음엔 `TZ=Asia/Seoul` 환경변수로 해결하려 했으나 **Vercel이 `TZ`를 예약어로 막아둔다.**
결국 `utils/dateRange.ts`의 `todayISO`/`currentMonth`가 `Intl.DateTimeFormat`으로 항상
한국 시간을 계산하도록 바꿨다. 환경 설정에 기대지 않으니 어느 서버에서 돌든 결과가 같고,
로컬과 배포가 갈리지 않는다는 점에서 원래 이 편이 낫다.
`TransactionForm`의 날짜 기본값도 같은 함수를 쓰도록 통일했다.

## 2026-07-21 — 처음 로그인하면 이 기기의 기록을 옮길지 묻는다 (Phase 8-5)

8-3/8-4 직후에는 로그인하면 화면이 비어 보였다. 로컬에 쌓인 기록이 계정으로 넘어가지
않았기 때문이다. 그 빈 화면 자리에 이전 안내를 띄우는 것으로 마무리했다.

- `POST /api/migrate` — 로컬 데이터 전체를 한 번에 받아 하나의 트랜잭션으로 넣는다.
  카테고리·규칙 id를 새로 발급하고 그걸 참조하는 거래·예산을 전부 갈아끼운다.
  **빈 계정에만 허용**하고 이미 데이터가 있으면 409로 거절한다(중복 방지).
  삽입 순서는 FK 때문에 카테고리 → 반복규칙 → 거래 → 예산으로 고정이다.
- `storage/migration.ts` — 옮길 게 있는지 판단하고 실행한다. 거래·예산·고정거래가
  전부 비어 있으면(기본 카테고리만 있는 상태) 안내를 띄우지 않는다.
- `components/MigrationSheet.tsx` — 옮길 항목 개수를 보여주고 "계정으로 옮기기 / 나중에".
- 기본 카테고리 시드를 `loadAll`에서 빼내 이전 결정 이후로 미뤘다. 시드부터 하면
  기본 카테고리와 옮겨온 카테고리가 겹친다.
- `storage/http.ts` — API 호출 helper를 repository에서 분리(migration도 쓴다).

**로컬 원본은 옮긴 뒤에도 지우지 않는다.** 로그아웃하면 쓰던 화면이 그대로 돌아온다.

**알려진 한계**: "나중에"를 고르면 기본 카테고리가 시드되어 계정이 더 이상 비어 있지 않게 되고,
그 뒤로는 이전 안내가 다시 뜨지 않는다. 나중에 마음이 바뀌면 옮길 방법이 없다.
