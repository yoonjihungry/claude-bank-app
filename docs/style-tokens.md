...
| `--card` | `0 0% 100%` | `#ffffff` | 카드·헤더 등 표면 배경 |
| `--card-foreground` | `222 47% 11%` | `#0f1729` | 카드 위 텍스트 (= foreground) |
| `--muted` | `214 32% 91%` | `#e1e7ef` | 옅은 회색 표면 (진행 바 트랙, 보조 버튼, hover) |
| `--muted-foreground` | `215 16% 47%` | `#65758b` | 보조/약한 텍스트 (라벨, 날짜, 건수) |
| `--input` | `214 32% 85%` | `#cdd7e5` | 입력창·컨트롤 테두리 |
| `--ring` | `221 70% 46%` | `#2357c7` | 포커스 링 (= primary) |
...
- **`--card` (+`-foreground`)** — 카드·헤더 등 배경 위에 떠 있는 표면. 흰 배경이 필요하면 `bg-white`가 아니라 `bg-card`를 쓴다.
- **`--muted` / `--muted-foreground`** — `muted`는 옅은 회색 표면(진행 바 트랙, 보조 버튼, hover 배경), `muted-foreground`는 강조가 필요 없는 보조 텍스트(라벨, 날짜, 건수). 기존의 `text-gray-*`는 여기로 대체한다.
- **`--input`** — 입력창·셀렉트·컨트롤의 테두리(`border-input`). **`--ring`** — 포커스 링(`focus:ring-ring`, `focus:border-ring`).
